...
class ADEPTDataset(Dataset):
    """Dataset for loading ADEPT videos."""
    ...
    def __init__(
        ...
        self.video_path = os.path.join(data_root, 'videos', 'train2') # split
        if self.split == 'test':
            self.video_path = os.path.join(data_root, 'videos', 'control')
            video_len = 175
        self.anno_path = os.path.join(data_root, 'annotations', split)

        self.adept_transforms = adept_transforms
    ...
    def _read_frames(self, idx):
        ...
        # otherwise, read from saved video frames
            ...
            if len(os.listdir(frame_dir)) != self.video_len:
                raise ValueError
            # list all jpg files in the frame_dir and sort them
            frames_path = sorted([
                os.path.join(frame_dir, f)
                for f in os.listdir(frame_dir)
                if f.endswith('.jpg')
            ])
            # select frames according to start_idx and frame_offset
            frames_indices = [start_idx + n * self.frame_offset for n in range(self.n_sample_frames)]
            frames_path = [frames_path[start_idx + n * self.frame_offset] for n in range(self.n_sample_frames)]
            frames = [read_img(frame_path) for frame_path in frames_path]
        ...
        if any(frame is None for frame in frames):
            raise ValueError
        frames = [
            self.adept_transforms(Image.fromarray(img).convert('RGB'))
            for img in frames
        ]  # [T, C, H, W]
    ...
    def _read_masks(self, idx):
        ...
        masks = [self.adept_transforms.process_mask(mask)
    ...
    def get_video(self, video_idx):
        ...
        if len(video) != self.video_len or any(img is None for img in video):
            ...
            return data_dict
        video = [
            self.adept_transforms(Image.fromarray(frame).convert('RGB'))
    ...
    def _get_files(self):
        ...
        for i in range(start, end):
            if self.split == 'test':
                video_paths.append(os.path.join(self.video_path, f'control_{i}.mp4'))
            else:
                video_paths.append(os.path.join(self.video_path, f'train_{i:05d}.mp4'))
    ...
    def _get_filtered_sample_idx(self):
        ...
        if False:
            # add self.files info to valid_idx
            for i,item in enumerate(valid_idx):
                video_idx, start_idx = item
                video_folder_number = int(self.files[video_idx].split('/')[-1].split('.')[0].split('_')[-1])
                valid_idx[i] = (video_idx, start_idx, video_folder_number)

            # store the valid idx for later use in a pickle file
            with open(f'valid_idx_{self.split}.pt', 'wb') as f:
                pickle.dump(valid_idx, f)
            
        return valid_idx


class ADEPTSlotsDataset(ADEPTDataset):
    """Dataset for loading ADEPT videos and pre-computed slots."""
    ...
    def __init__(
        ...
        super().__init__(
            data_root=data_root,
            adept_transforms=adept_transforms,
            split=split,
    ...
    def __getitem__(self, idx):
        """Data dict:
            - data_idx: int
            - img: [T, 3, H, W]
            - slots: [T, N, C] slots extracted from ADEPT video frames
            - error_flag: whether loading `idx` causes error and _rand_another
            - mask: [T, H, W], 0 is background
            - pres_mask: [T, max_num_obj], valid index of objects
            - bbox: [T, max_num_obj, 4], object bbox padded to `max_num_obj`
...
def build_adept_dataset(params, val_only=False, test_set=False):
    """Build ADEPT video dataset."""
    args = dict(
        data_root=params.data_root,
        adept_transforms=BaseTransforms(params.resolution),
        split='val',
        max_n_objects=6,
    ...
    if test_set:
        assert not val_only
        args['split'] = 'test'
        return ADEPTDataset(**args)

    val_dataset = ADEPTDataset(**args)
    if val_only:
        return val_dataset
    args['split'] = 'train'
    train_dataset = ADEPTDataset(**args)
    return train_dataset, val_dataset


def build_adept_slots_dataset(params, val_only=False):
    """Build ADEPT video dataset with pre-computed slots."""
    ...
    val_dataset = ADEPTSlotsDataset(**args)
    if val_only:
        return val_dataset
    args['split'] = 'train'
    args['video_slots'] = slots['train']
    train_dataset = ADEPTSlotsDataset(**args)
    return train_dataset, val_dataset
