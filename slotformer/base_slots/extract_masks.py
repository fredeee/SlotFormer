...
@torch.no_grad()
def extract_video_slots(model, dataset, first_n_samples=1):
    ...
    max_videos = min(first_n_samples, dataset.num_videos)
    range_idx = range(0, min(first_n_samples, dataset.num_videos), bs)
    ...
    for start_idx in tqdm(range_idx):
        ...
        out_dict = model(in_dict, get_rawmasks = True)
        slots = out_dict['post_masks'].detach().cpu().numpy()  # [B, T, n, c]
        all_slots += [slot for slot in slots]
        slots_raw = out_dict['raw_masks'].detach().cpu().numpy()  # [B, T, n, c]
        all_slots_raw += [slot for slot in slots_raw]
        torch.cuda.empty_cache()
    all_slots = np.stack(all_slots, axis=0)  # [N, T, n, c]
    all_slots_raw = np.stack(all_slots_raw, axis=0)  # [N, T, n, c]
    return all_slots, all_slots_raw
...
def process_video(model):
    ...
    val_slots, val_slots_raw = extract_video_slots(model, val_set)
    print(f'Processing {params.dataset} video train set...')
    train_slots, train_slots_raw = extract_video_slots(model, train_set)

    # also extract test_set for CLEVRER
    if params.dataset == 'adept':
        test_set = build_adept_dataset(params, test_set=True)
        print(f'Processing {params.dataset} video test set...')
        test_slots, test_slots_raw = extract_video_slots(model, test_set, first_n_samples=200)
    ...
    try:
        ...
        train_slots_raw = {
            os.path.basename(train_set.files[i]): train_slots_raw[i]
            for i in range(len(train_slots_raw))
        }  # each embedding is of shape [T, N, C]
        val_slots_raw = {
            os.path.basename(val_set.files[i]): val_slots_raw[i]
            for i in range(len(val_slots_raw))
        }
        slots = {'train': train_slots, 'val': val_slots, 'train_raw': train_slots_raw, 'val_raw': val_slots_raw}
        ...
        if params.dataset == 'adept':
            ...
            test_slots_raw = {
                os.path.basename(test_set.files[i]): test_slots_raw[i]
                for i in range(len(test_slots_raw))
            }
            slots['test'] = test_slots
            slots['test_raw'] = test_slots_raw
...
def main():
    ...
    model.testing = False #True  # we just want slots
